...
function DynamoRestore(options) {
    ...
    options.awsSession = options.awsSession || process.env.AWS_SESSION_TOKEN;

    AWS.config.update({
        accessKeyId: options.awsKey,
        secretAccessKey: options.awsSecret,
        sessionToken: options.awsSession,
...
DynamoRestore.prototype._checkTableExists = function(error, data) {
    ...
    if (data.TableNames.indexOf(this.options.table) > -1) {
        ...
        if (this.options.overwrite) {
            ...
            setTimeout(dynamodb.describeTable.bind(dynamodb, { TableName: this.options.table }, this._checkTableReady.bind(this)), 5000);
        } else {
    ...
    if (this.options.partitionkey && this.options.partitionkeytype) {
        ...
        setTimeout(this._checkTableExists.bind(this, error, data), 1000);
    }
};
...
DynamoRestore.prototype._checkTableReady = function(error, data) {
    ...
    if (data &&
        ...
        setTimeout(dynamodb.describeTable.bind(dynamodb, { TableName: data.Table.TableName }, this._checkTableReady.bind(this)), 5000);
    }
};
...
DynamoRestore.prototype._finishBatches = function() {
    ...
    setTimeout(this._finishBatches.bind(this), 1000);
};
